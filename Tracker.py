...
from data.account_id import account_id
...
def do_something(sc, iterations=[], subs_history=[], likes_history=[]):
    ...
    plt.savefig('plots/subs_fig.png')
    ...
    plt.ylabel('Likes')
    plt.xlabel('Time (minutes)')
    plt.savefig('plots/likes_fig.png')

    # Save data
    np.savetxt('data/likes.csv', likes_history, delimiter=',')
    np.savetxt('data/subs.csv', subs_history, delimiter=',')
    np.savetxt('data/iterations.csv', iterations, delimiter=',')
...
if __name__ == '__main__':
    print('File loaded!')
    api = TikTokAPI()
    iters = np.loadtxt('data/iterations.csv')
    subs_hist = np.loadtxt('data/subs.csv')
    likes_hist = np.loadtxt('data/likes.csv')
